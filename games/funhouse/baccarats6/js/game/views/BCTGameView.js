...
export class BCTGameView extends TGGameView {
    ...
    getCountText (data) {
        return (data.createGame.roundCount + " / " + data.createGame.totalCount);
    }
    onBCTSocketMessage (data) {
        ...
        if (msg.indexOf("easySix") > -1 || msg.indexOf("vip") > -1) {
            //console.log("REturning for easy six == " + msg)
            return;
        }
        if (msg != "BaccaratBet") {
            this.cardsAreaComp.updateGameStatus(msg);
            //console.log(data);
        } else {
        ...
        switch (msg) {
            ...
            case "startGame" :
                ...
                console.log("Start game == ", CoreLib.Model.GameConfig.bctWinning)
                this.panelComp.setState(false);
                ...
                break;
            case "gameResult" :
                if (data.gameResult.player_id == slotModel.getPlayerId()) {
                    CoreLib.Model.GameConfig.bctWinning += data.gameResult.result.totalWin;
                    console.log("win == " + CoreLib.Model.GameConfig.bctWinning)
                    CoreLib.Model.GameConfig.playedGame = true;
                    CoreLib.WrapperService.requestUserBalance();
                    //slotModel.updateBaccaratBalance(data);
                    this.cardsAreaComp.storeGameResult(data.gameResult);
                }
                break;
            case "runGame" :
                this.panelComp.setState(false);
                ...
                this.chipsPanel.setCompEnabled(false);
                this.dealResult = {};
                this.dealResult.result = data.runGame;
                this.panelComp.setState(false);
                this.cardsAreaComp.dealCards(this.dealResult.result);
                break;
            case "BaccaratBet" :
                this.handleBetPlacement(data.BaccaratBet);
                break;
            default :
                break;
        }
