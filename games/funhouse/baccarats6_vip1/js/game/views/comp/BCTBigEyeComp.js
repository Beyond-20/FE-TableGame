...
import { BCTBigEyeElement } from './BCTBigEyeElement'

let columns = 20;
let rows = 10;

export class BCTBigEyeComp extends LibView {
    constructor(config) {
        super(config);
        this.createItems();
    }
        ...
        for (let k = start; k < len; k++) {
            ...
            for (let i = 0; i < len2; i++) {
                if (i == 0) {
                    ...
                    if (result[k - 1].length == result[k - 2].length) {
                        if (lastcol == "blue") {
                            index++
                            if (data[index] == undefined) {
                                data[index] = [];
                    ...
                    if (result[k - 1][i] == result[k - 1][i - 1]) {
                        if (lastcol == "blue") {
                            index++
                            if (data[index] == undefined) {
                                data[index] = [];
        ...
        for (let k = 0; k < newlen; k++) {
            ...
            if (newlen2 > 0) {
                for (let i = 0; i < newlen2; i++) {
                    if (i <= rows - 1) {
                       this.items2DArray[k][i].updateBigEyeElement(data[k][i]);
                    } else {
                        if (this.items2DArray[k + trailIndex] && this.items2DArray[k + trailIndex][rows - 1]) {
                            this.items2DArray[k + trailIndex][rows - 1].updateBigEyeElement(data[k][i]);
                            trailIndex++;
                        }
